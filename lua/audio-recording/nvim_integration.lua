...
function M.setup(core_module)
  ...
  vim.api.nvim_create_user_command('Rec', function(opt)
    ...
    if cmd == 'start' then core_module:start_recording()
    elseif cmd == 'stop' then core_module:stop_recording()
    elseif cmd == 'annotate' then core_module:annotate()
    else
      vim.notify('Uso: :Rec start|stop|annotate', vim.log.levels.INFO)
    end
  end, {
    nargs = '*',
    force = true,
